...
./build/Debug/erkao.exe pkg add ../path/to/pkg --range ^1.2.0
./build/Debug/erkao.exe pkg add ../path/to/pkg --global
./build/Debug/erkao.exe pkg install
./build/Debug/erkao.exe pkg install --update
./build/Debug/erkao.exe pkg update
...
- If a module contains any `export` statements, only exported names are exposed.
  Otherwise all top-level bindings are exposed except those marked `private`.
...
Mark a top-level declaration as private to hide it from implicit exports:

```ek
private let secret = 123;
```

`private` is only allowed at the top level.
...
Lockfile (`erkao.lock`), generated by `pkg install --update` (or the first `pkg install`):
...
and comparisons like `>=1.0.0 <2.0.0`. `pkg install --update` resolves ranges to
the highest available version in `packages/` or the global cache, then writes
exact versions into `erkao.lock`. `pkg install` uses the lockfile if present.
...
Use `--range` to record a version range in `erkao.mod` and refresh `erkao.lock`
with `pkg install --update` (or `pkg update`).
...
`and`, `as`, `break`, `case`, `class`, `const`, `continue`, `default`, `else`, `enum`, `export`, `false`, `for`, `foreach`, `from`, `fun`, `if`, `import`, `in`, `let`, `match`, `null`, `or`, `private`, `return`, `switch`, `this`, `true`, `while`
